        ...
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_0_4:
                    npcSay(player, ConstTask.NPC_NHA,
                            "Ok ngon chim rồi\n"
                            + "Giờ con đi đánh cho ta 5 mộc nhân..\n"
                            + "..sau đó về báo cáo cho ta!");
                    break;
...
//        }
    ...
    private boolean isCurrentTask(Player player, int idTaskCustom) {
        return player.playerTask != null && player.playerTask.taskMain != null && idTaskCustom == (player.playerTask.taskMain.id << 10) + player.playerTask.taskMain.index << 1;
    }
    ...
    public void paySideTask(Player player) {
        if (player.playerTask.sideTask.template != null) {
            if (player.playerTask.sideTask.isDone()) {
                ...
                Item item = ItemService.gI().createNewItem((short) (ConstDataEvent.ID_ITEM_6));
                item.quantity = goldReward;
                item.itemOptions.add(new Item.ItemOption(30, 1));

                if (InventoryServiceNew.gI().getCountEmptyBag(player) <= 0) {
                    player.inventory.itemsMailBox.add(item);
                    if (GodGK.updateMailBox(player)) {
                        Service.gI().sendThongBao(player, "Hành trang đầy, quà đã được gửi về hòm thư");
                    }
                } else {
                    InventoryServiceNew.gI().addItemBag(player, item);
                }
...
//    }
    public void checkDoneSideTaskPickItem(Player player, ItemMap item) {
        if (player.playerTask != null && player.playerTask.sideTask != null) {
            if (player.playerTask.sideTask.template != null) {
                if (player.playerTask.sideTask.template.id == 58 && item != null && item.itemTemplate != null && item.itemTemplate.type == 9) {
                    player.playerTask.sideTask.count += item.quantity;
                    notifyProcessSideTask(player);
                }
            }
