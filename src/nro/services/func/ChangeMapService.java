...
import nro.models.player.EffectSkill;
...
public class ChangeMapService {
    ...
    private static final byte EFFECT_GO_TO_CDRD = 2;
    ...
    public static final byte DEFAULT_SPACE_SHIP2 = 4;
        ...
        try {
            ...
            switch (pl.iDMark.getTypeChangeMap()) {
                ...
                case ConstMap.CHANGE_BLACK_BALL:
                    ...
                    break;
                case ConstMap.CHANGE_GIAI_DAU_BALL:
                    list = (pl.mapGiaidauBall != null ? pl.mapGiaidauBall
                            : (pl.mapGiaidauBall = MapService.gI().getMapGiaidauBall()));
                    msg.writer().writeByte(list.size());
                    for (Zone zone : list) {
                        msg.writer().writeUTF(zone.map.mapName);
                        msg.writer().writeUTF(zone.map.planetName);
...
//end khaile comment
            ...
            if (pl.zone != null) {
                ...
                if (!pl.isBoss && !pl.isPet && !pl.isNewPet && !pl.isMiniPet) {
...
//            checkJoinnguhs(pl);
    ...
    public void changeMapWaypoint(Player player) {
        if (player == null || player.location == null || player.zone == null || player.zone.map == null) {
            return;
        }
        Zone zoneJoin = null;
        WayPoint wp = null;
        ...
        if (zoneJoin != null) {
            ...
            resetPoint(player);
            Service.gI().sendThongBao(player, "Tiêu diệt hết tất cả trước khi qua ải tiếp theo!");
        }
...
//        Logger.log(Logger.BLUE, "Bạn " + player.name + " đang ở " + player.zone.map.mapName + " khu " + player.zone.zoneId + "\n");
    }
    ...
    private void sendEffectMeToMap(Player player) {
        ...
        try {
            ...
            if (player.mobMe != null) {
                ...
                msg.writeFix(Util.maxIntValue(player.mobMe.point.gethp()));// hp mob
                Service.gI().sendMessAnotherNotMeInMap(player, msg);
                msg.cleanup();
            }
            if (player.pet != null && player.pet.mobMe != null) {
                ...
                msg.writeFix(Util.maxIntValue(player.pet.mobMe.point.gethp())
                );// hp mob
                Service.gI().sendMessAnotherNotMeInMap(player, msg);
                msg.cleanup();
            }
        ...
        try {
            ...
            for (Player pl : players) {
                if (!player.equals(pl)) {
                    ...
                    EffectSkill effectSkill = pl.effectSkill;
                    if (effectSkill != null) {
                        ...
                        if (effectSkill.isThoiMien) {
                            msg = new Message(-124);
                            msg.writer().writeByte(1);
                            msg.writer().writeByte(0);
                            msg.writer().writeByte(41);
                            msg.writer().writeInt((int) pl.id);
                            player.sendMessage(msg);
                            msg.cleanup();
                        }
                        if (effectSkill.isBlindDCTT || effectSkill.isStun) {
                            msg = new Message(-124);
                            msg.writer().writeByte(1);
                            msg.writer().writeByte(0);
                            msg.writer().writeByte(40);
                        ...
                        if (effectSkill.isBang || effectSkill.isBang) {
                            msg = new Message(-124);
                            msg.writer().writeByte(1);
                            msg.writer().writeByte(0);
                            msg.writer().writeByte(202);
                            msg.writer().writeInt((int) pl.id);
                            player.sendMessage(msg);
                            msg.cleanup();
                        }
                        if (effectSkill.useTroi) {
                            if (effectSkill.plAnTroi != null) {
                                msg = new Message(-124);
                                msg.writer().writeByte(1);
                                msg.writer().writeByte(0);
                                msg.writer().writeByte(32);
                                msg.writer().writeInt((int) effectSkill.plAnTroi.id);
                                msg.writer().writeInt((int) pl.id);
                                player.sendMessage(msg);
                                msg.cleanup();
                            }
                            if (effectSkill.mobAnTroi != null) {
                                msg = new Message(-124);
                                msg.writer().writeByte(1);
                                msg.writer().writeByte(1);
                                msg.writer().writeByte(32);
                                msg.writer().writeByte(effectSkill.mobAnTroi.id);
                                msg.writer().writeInt((int) pl.id);
                                player.sendMessage(msg);
                                msg.cleanup();
                            }
                    ...
                    if (pl.mobMe != null) {
                        ...
                        msg.writeFix(Util.maxIntValue(pl.mobMe.point.gethp()));
                        player.sendMessage(msg);
                        msg.cleanup();
                    }
                }
...
//                    Service.gI().sendMessAllPlayerIgnoreMe(player, msg);
    ...
    public void spaceShipArrive2(Player player, byte typeSendMSG, byte typeSpace) {
        Message msg;
        try {
            msg = new Message(-65);
            msg.writer().writeInt((int) player.id);
...
//                    Service.gI().sendMessAllPlayerIgnoreMe(player, msg);
        ...
        changeMap(player, null, 135, -1, 79, 5, DEFAULT_SPACE_SHIP);
    ...
    public void goToMap(Player player, Zone zoneJoin) {
        ...
        //Cannot invoke "nro.models.map.Zone.addPlayer(nro.models.player.Player)" because "player.zone" is null
        if (player.zone == null) {
            return;
        }
...
//            player.maxTime = 10;
    ...
    public void goToCDRD(Player player) {
        if (!player.iDMark.isGoToCDRD()) {
            ...
            spaceShipArrive(player, (byte) 1, DEFAULT_SPACE_SHIP);
            effectChangeMap(player, 60, EFFECT_GO_TO_BDKB);
//         ChangeMapService.this.changeMapBySpaceShip(player, 141, -1, -1);
        }
    }
...
//  private void checkJoinnguhs(Player player) {
...
//  }
    ...
    public void goToHanhTinhThucVat(Player player) {
        ChangeMapService.this.changeMapBySpaceShip(player, 160, -1, -1);
    }
...
//            return null;
//        }
        if (player.isPet || player.isMiniPet || player.isBoss || player.getSession() != null && player.isAdmin()) {
            return zoneJoin;
        }
        ...
        if (zoneJoin != null) {
            switch (zoneJoin.map.mapId) {
                ...
                case 170: // Đảo SkyPiea
                    if (player.getSession().player.nPoint.power >= 5000000L) {
        ...
        if (player != null && player.zone != null) {
            switch (player.zone.map.mapId) {
                ...
                case 91:
                    ...
                    break;
                case 145:
                    Giaidauvutru.gI().joinMapGiaidauWar(player);
                    break;
            }
        }
    }
