...
public class InventoryServiceNew {
    ...
    public static InventoryServiceNew getInstance() {
        if (InventoryServiceNew.I == null) {
            InventoryServiceNew.I = new InventoryServiceNew();
        }
        return InventoryServiceNew.I;
        ...
        try {
            if (where == 0) {
                if (index < player.inventory.itemsBody.size()) {
                    ...
                    System.out.println("Invalid index for itemsBody");
                }
            } else if (where == 1) {
                if (index < player.inventory.itemsBag.size()) {
                    ...
                    if (itemThrow != null && itemThrow.template != null) {
                        ...
                        Service.gI().sendThongBao(player, "" + ItemService.gI().getTemplate(index) + " không tồn tại");
                    }
                } else {
                    Service.gI().sendThongBao(player, "Ô thành trang không tồn tại");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
...
//            }
    ...
    public void itemBoxToBodyOrBag(Player player, int index) {
        if (index >= 0 && index < player.inventory.itemsBox.size()) {
            ...
            Service.gI().sendThongBao(player, "Cos lỗi xảy ra với rương đồ");
            System.out.println("Invalid index box: " + index);
        }
    }
        ...
        if (index >= 0 && index < player.inventory.itemsBag.size()) {
            ...
            if (item != null) {
                if (item.template.id == 457) {
                    Service.gI().sendThongBaoOK(player, "Không thể cất vàng vào rương");
                    return;
                }
            ...
            Service.gI().sendThongBao(player, "Cos lỗi xảy ra với hành trang");
            System.out.println("Invalid index bag: " + index);
        }
    }
...
//                return true;
//            case 211: //nho tim
//                player.nPoint.setFullStamina();
//                PlayerService.gI().sendMaxStamina(player);
//                return true;
//            case 212: //nho xanh
//                player.nPoint.setF20Stamina();
...
//                return true;
    ...
    public boolean addItemBag(Player player, Item item) {
        ...
        if (ItemMapService.gI().isGiaidauBall(item.template.id)) {
            return Giaidauvutru.gI().pickGiaidauBall(player, item);
        }
        ...
        if (itemAdd.template.isUpToUp) {
            for (Item it : items) {
                ...
                if (it.template.id == itemAdd.template.id && it.haveOption(93)) {
                    ItemOption opt = it.getOption(93);
                    for (ItemOption itop : itemAdd.itemOptions) {
                        if (itop.optionTemplate.id == 93) {
                            itop = opt;
                            break;
                        }
                    }
                ...
                if (itemAdd.template.id == 447
                        || itemAdd.template.type == 93
                        || itemAdd.template.id == 457
                        || itemAdd.template.id == 590
                        //Mảnh bông tai
                        || itemAdd.template.id == 933
                        || itemAdd.template.id == 934
                        || itemAdd.template.id == 2120 //Thiên Sứ
                        || itemAdd.template.id == 2227) {
                ...
                if (it.quantity < 10_000_000) { // Thay 999 → 10_000_000
                    int add = 10_000_000 - it.quantity;
...
//end khaile modify
//                if (itemAdd.template.id == 890 
//                        || itemAdd.template.id == 891
//                        ) {
//                    if (it.quantity < 999) {
...
//                }
    ...
    private boolean isItemIncremental(Item item) { //item cộng dồn số lượng
        ...
        if (temp == 457 || temp == 372 || temp == 373 || temp == 374 || temp == 375 || temp == 376 || temp == 377 || temp == 378 || temp == 193 || temp == 225 || temp == 402 || temp == 403 || temp == 404 || temp == 759 || temp == 361 || temp == 73) {
            return true;
        }
        return false;
    }
...
//
//            //item cộng thêm chỉ số param: tự động luyện tập
//            int[] idParam = isItemIncrementalOption(item);
//            if (idParam[0] != -1) {
//                for (Item it : items) {
...
//                        continue;
//                    }
//                    if (it.template.id == item.template.id && it.haveOption(93)) {
//                        ItemOption opt = it.getOption(93);
//                        for (ItemOption itop : item.itemOptions) {
//                            if (itop.optionTemplate.id == 93) {
//                                itop = opt;
//                                break;
//                            }
...
//
//    private boolean isItemIncremental(Item item) { //item cộng dồn số lượng
//        if (item.template.id == 964 ||item.template.id == 965 ||item.template.id == 1213 || item.template.id == 1199 || item.template.id == 1055 || item.template.id == 590 || item.template.id == 521 || item.template.id == 933 || item.template.id == 942 || item.template.id == 943 || item.template.id == 944) {
//            return false;
//        }
//        if (item.template.type == 14 || item.template.type == 22 || item.template.type == 12 || item.template.type == 30 || item.template.type == 31 || item.template.type == 33 || item.template.type == 6 || item.template.type == 27) {
//            return true;
//        }
//        int temp = item.template.id;
//        if (temp == 457 || temp == 372 || temp == 373 || temp == 374 || temp == 375 || temp == 376 || temp == 377 || temp == 378 || temp == 193 || temp == 225 || temp == 402 || temp == 403 || temp == 404 || temp == 759 || temp == 361 || temp == 73) {
...
//    }
    ...
    public Item finditem(Player player, int TemdId, int quantity) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == TemdId) && item.quantity >= quantity) {
                return item;
            }
        }
        return null;
    ...
    public Item finditemnuochoa(Player player, int soluong) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 1380) && item.quantity >= soluong) {
                return item;
            }
        }
        return null;
    ...
    public Item finditemnuochoaVIP(Player player, int soluong) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 1381) && item.quantity >= soluong) {
                return item;
            }
        }
        return null;
