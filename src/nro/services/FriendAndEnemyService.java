    ...
    private void reloadFriend(Player player) {
        for (Friend f : player.friends) {
            ...
            if ((pl = Client.gI().getPlayerByUser(f.id)) != null || (pl = Client.gI().getPlayer(f.name)) != null) {
                try {
                    if (pl.nPoint != null) { // Kiểm tra nPoint có khả dụng hay không
            ...
            if (pl != null) {
                if (player.isAdmin() || player.nPoint.teleport) {
                    if (!pl.itemTime.isUseAnDanh || player.isAdmin()) {
                        if ((player.isAdmin() || !pl.zone.isFullPlayer())&& !MapService.gI().isMapBanDoKhoBau(pl.zone.map.mapId)&& !MapService.gI().isMapConDuongRanDoc(pl.zone.map.mapId) && !MapService.gI().isMapDoanhTrai(pl.zone.map.mapId) && !MapService.gI().isNguHanhSon(pl.zone.map.mapId) && !MapService.gI().isMapMaBu(pl.zone.map.mapId)) {
