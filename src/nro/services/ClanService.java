...
import nro.models.Template.FlagBag;
...
import static nro.models.item.ItemTime.CON_DUONG_RAN_DOC;
...
//                                 plGive.achievement.plusCount(9);
    ...
    private void askForPea(Player player) {
        ...
        if (clan != null) {
            ...
            long timeRemainingMillis = cm.timeAskPea + 1000 * 60 * 5 - currentTimeMillis;
            ...
            if (cm != null) {
                if ((cm.timeAskPea + 1000 * 60 * 5) < System.currentTimeMillis()) {
                    ...
                    Service.gI().sendThongBao(player, "Bạn chỉ còn " + timeRemainingSeconds + " giây để xin đậu");
                }
            }
        }
    }
            ...
            if (player.clan == null) {
                ...
                if (clan != null) {
                    ...
                    if (!isMeInClan) {
                        ...
                        Service.gI().sendThongBao(player, "Lỗi xảy ra tại bang hội");
                    }
                }
            } else {
    ...
    public void leaveClan(Player player) {
        ...
        if (clan != null) {
            ...
            if (cm != null) {
                ...
                ItemTimeService.gI().removeTextConDuongRanDoc(player);
    ...
    public void kickOut(Player player, int memberId) {
        ...
        if (clan != null && cm != null
            ...
            if (plKicked != null) {
                ...
                ItemTimeService.gI().removeTextBDKB(plKicked);
                ItemTimeService.gI().removeTextConDuongRanDoc(plKicked);
            } else {
...
//                ClanDAO.updateClanMember(cm);
    ...
    public void update() {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ps = con.prepareStatement("update clan_sv" + Manager.SERVER
                    ...
                    + "level = ?, members = ? , doanh_trai = ?, ban_do_kho_bau= ?,ran_doc= ? where id = ? limit 1");
            for (Clan clan : Manager.CLANS) {
                ...
                String bandokhobau = "[";
                bandokhobau += clan.banDoKhoBau_lastTimeOpen + ",";
                bandokhobau += "\"" + clan.banDoKhoBau_playerOpen + "\"]";
                ps.setString(9, bandokhobau);

                String cdrd = "[";
                cdrd += clan.ConDuongRanDoc_lastTimeOpen + ",";
                cdrd += "\"" + clan.CDRD_playerOpen + "\"]";
                ps.setString(10, cdrd);
    ...
    public void close() {
        for (Clan clan : Manager.CLANS) {
            ...
            long timecdrd = clan.ConDuongRanDoc_lastTimeOpen;

            String doanhtrai = "[";
            doanhtrai += (timeDoanhtrai == 0 ? System.currentTimeMillis() : timeDoanhtrai) + ",";
            doanhtrai += "\"" + clan.doanhTrai_playerOpen + "\"]";
            String bandokhobau = "[";
            bandokhobau += (timebandokhobau == 0 ? System.currentTimeMillis() : timebandokhobau) + ",";
            bandokhobau += "\"" + clan.banDoKhoBau_playerOpen + "\"]";
            String cdrd = "[";
            cdrd += (timecdrd == 0 ? System.currentTimeMillis() : timecdrd) + ",";
            cdrd += "\"" + clan.CDRD_playerOpen + "\"]";
            String query = " update clan_sv" + Manager.SERVER
                    + " set slogan = ?,name = ?, img_id = ?, power_point = ?, max_member = ?, clan_point = ?,"
                    + "level = ?, members = ? ,doanh_trai = ?,ban_do_kho_bau = ?,ran_doc = ?  where id = ?";
            try {
