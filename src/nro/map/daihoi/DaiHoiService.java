...
//                    Util.log("RUN ROI NE: ");
                    ...
                    TimerTask OpenDHVT = new TimerTask() {
                        ...
                        public void run() {
                            ...
                            //                                Service.gI().sendThongBao(_p,"Trận đấu của bạn sẽ diễn ra trong vòng " + (int)((DaiHoiManager.gI().tOpenDHVT - System.currentTimeMillis())/60000) + " phút nữa");
                            //                            }
                            //                        }
                            Player _p = null;
                            ...
                            if (p11.zone.map.mapId != 51) {
                                ...
                                if (!p11.isDie() && !p22.isDie()) {
                                    ...
                                    if (p11.nPoint.hp >= p22.nPoint.hp) {
                                        ...
                                        Service.gI().sendThongBao(p11, "Bạn đã chiến thắng, bạn nhận được " + DaiHoiManager.gI().costRoundDHVT());
...
//                                        }
                                        ...
                                        Service.gI().hsChar(p22, 1, 1);
                                        ChangeMapService.gI().changeMapInYard(p22, 52, -1, -1);
                                        Service.gI().sendThongBao(p22, "Bạn đã thua, hẹn gặp lại ở giải sau");
                                    } else {
                                        //                                            DaiHoiManager.gI().lstPlayers.add(p22);
                                        DaiHoiManager.gI().lstIDPlayers.add(p22.id);
                                        Service.gI().sendThongBao(p22, "Bạn đã chiến thắng, bạn nhận được " + DaiHoiManager.gI().costRoundDHVT());
                                        //CHECK NHIEM VU VONG 2 DHVT
...
//                                        }
                                        ...
                                        ChangeMapService.gI().changeMapInYard(p22, 52, -1, -1);
                                        Service.gI().hsChar(p11, 1, 1);
                                        ChangeMapService.gI().changeMapInYard(p11, 52, -1, -1);
                                        Service.gI().sendThongBao(p11, "Bạn đã thua, hẹn gặp lại ở giải sau");
                                    }
                                    timerVS.cancel();
                                } else {
                                    timerVS.cancel();
            ...
            if (!DaiHoiManager.gI().lstPlayers.isEmpty()) {
                Service.gI().sendThongBao(Client.gI().getPlayer(DaiHoiManager.gI().lstIDPlayers.get(0)), "Bạn được vào vòng tiếp theo");
                //CHECK NHIEM VU VONG 2 DHVT
...
//                                DaiHoiManager.gI().lstPlayers2.clear();
                        ...
                        if (DaiHoiManager.gI().lstIDPlayers.size() > 1) {
                            ...
                            for (int i = 0; i < DaiHoiManager.gI().lstIDPlayers.size(); i++) {
                                _p = Client.gI().getPlayer(DaiHoiManager.gI().lstIDPlayers.get(i));
                                if (_p != null && _p.session != null) {
                                    Service.gI().sendThongBao(_p, "Trận đấu của bạn sẽ diễn ra trong vòng " + (int) ((DaiHoiManager.gI().tNextRound - System.currentTimeMillis()) / 1000) + " giây nữa");
                                }
                            }
                        } else {
                            ...
                            if (DaiHoiManager.gI().lstIDPlayers.size() == 1) {
                                ...
                                Service.gI().sendThongBao(Client.gI().getPlayer(DaiHoiManager.gI().lstIDPlayers.get(0)), "Bạn đã vô địch giải đấu, xin chúc mừng bạn, bạn được thưởng 5 viên đá nâng cấp");
                            }
                            DaiHoiManager.gI().lstIDPlayers.clear();
                            DaiHoiManager.gI().lstIDPlayers2.clear();
                            DaiHoiManager.gI().openDHVT = false;
            ...
            //THONG BAO VO DICH
            if (countPlayer == 1) {
                _p0 = Client.gI().getPlayer(DaiHoiManager.gI().lstIDPlayers.get(0));
                if (_p0 != null && _p0.session != null) {
                    Service.gI().sendThongBao(_p0, "Bạn đã vô địch giải đấu, xin chúc mừng bạn, bạn được thưởng 100 ngọc");
...
//                    Util.log("KHONG VAO DAY DAU YEN TAM");
    ...
    public void winRoundDHVT(Player pW, Player pL) {
        if (pW != null && pW.session != null) {
            Service.gI().sendThongBao(pW, "Đối thủ đã kiệt sức hoặc rơi đài, bạn đã thắng");
            Service.gI().sendThongBao(pW, "Bạn vừa nhận thưởng " + DaiHoiManager.gI().costRoundDHVT());
            //CHECK NHIEM VU VONG 2 DHVT
        ...
        TimerTask winDHVT = new TimerTask() {
            ...
            public void run() {
                ...
                if (pL != null && pL.session != null) {
                    ...
                    Service.gI().sendThongBao(pL, "Bạn đã thua, hẹn gặp lại ở giải sau");
                }
            }
        ;
        };
