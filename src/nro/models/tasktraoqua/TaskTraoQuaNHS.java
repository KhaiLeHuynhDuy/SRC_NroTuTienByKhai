...
public class TaskTraoQuaNHS extends TimerTask {
...
//        }, getDelayUntilMidnight(), 24, TimeUnit.HOURS);
    ...
    private static void traoQuaTopSieuHang() {
        ...
        try (Connection con = GirlkunDB.getConnection()) {
            PreparedStatement ps = con.prepareStatement("SELECT id, name, event_point_nhs FROM player ORDER BY event_point_nhs DESC LIMIT 10;");
            ...
            for (int i = 0; i < arrayList.size(); i++) {
                ...
                Logger.success("Dang trao qua top NHS" + i);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static String[] Text = new String[]{
        "|3|Top 1:"
        + "x" + 3 + " " + ItemService.gI().getTemplate(2125).name + ", \n"
        + "|2|Top 2:"
        + "x" + 2 + " " + ItemService.gI().getTemplate(2125).name + ", \n"
        + "|1|Top 3:"
        + "x" + 2 + " " + ItemService.gI().getTemplate(2125).name + ", \n"
        + "|4|Top 4: "
        + "x" + 1 + " " + ItemService.gI().getTemplate(2125).name + ", \n"
        + "|6|Top 5: "
    ...
    private static void traoQuaTopI(int i, int id) {
        try {
            ...
            switch (i) {
                case 0: {
                    Item item = ItemService.gI().createNewItem((short) 2125);
                    item.quantity = 3;
                    item.itemOptions.add(new Item.ItemOption(30, 1));
                    pl.inventory.itemsMailBox.add(item);
                    GodGK.updateMailBox(pl);
                       Logger.error("Đã trao quà top " + (i + 1) + " NHS cho: " + pl.name);
                    break;
                }
                case 1: {
                    Item item = ItemService.gI().createNewItem((short) 2125);
