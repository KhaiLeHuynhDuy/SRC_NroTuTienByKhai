...
public class TaskTraoQuaTV extends TimerTask {

    public static boolean isDaTraoQuaTop = true;
    public static int THOIVANG = 457;
    ...
    private static void traoQuaTopSieuHang() {
        ...
        try (Connection con = GirlkunDB.getConnection()) {
            PreparedStatement ps = con.prepareStatement("SELECT \n"
                    ...
                    + "          CONCAT(\n"
                    + "            '[" + THOIVANG + ",',\n"
                    + "            SUBSTRING_INDEX(SUBSTRING_INDEX(player.items_bag, '[" + THOIVANG + ",', -1), ']', 1)\n"
                    + "          ),\n"
                    + "          ',',\n"
                    + "          2\n"
                    + "        ),\n"
                    ...
                    + "          CONCAT(\n"
                    + "            '[" + THOIVANG + ",',\n"
                    + "            SUBSTRING_INDEX(SUBSTRING_INDEX(player.items_box, '[" + THOIVANG + ",', -1), ']', 1)\n"
                    + "          ),\n"
                    + "          ',',\n"
                    + "          2\n"
                    + "        ),\n"
                    ...
                    + "WHERE\n"
                    + "  (\n"
                    + "    player.items_box LIKE '%\\\"[" + THOIVANG + ",%'\n"
                    + "    OR player.items_bag LIKE '%\\\"[" + THOIVANG + ",%'\n"
                    + "  )\n"
                    + "ORDER BY\n"
                    + "  thoi_vang DESC\n"
                    + "LIMIT\n"
            ...
            for (int i = 0; i < arrayList.size(); i++) {
                ...
                Logger.success("Dang trao qua top thoiVang_ " + i);
            }
        } catch (Exception e) {
        }
    }
...
//WHERE
//  (
//    player.items_box LIKE CONCAT('%\"[', @thoivang, ',%')
//    OR player.items_bag LIKE CONCAT('%\"[', @thoivang, ',%')
//  )
//ORDER BY
//  thoi_vang DESC
//LIMIT
//  10;
    ...
    public static String[] Text = new String[]{
        "|3|Top 1:"
        + "x" + 10 + " " + ItemService.gI().getTemplate(457).name + ", \n"
        + "|2|Top 2:"
        + "x" + 9 + " " + ItemService.gI().getTemplate(457).name + ", \n"
        + "|1|Top 3:"
        + "x" + 8 + " " + ItemService.gI().getTemplate(457).name + ", \n"
        + "|4|Top 4: "
        + "x" + 7 + " " + ItemService.gI().getTemplate(457).name + ", \n"
        + "|6|Top 5: "
        + "x" + 6 + " " + ItemService.gI().getTemplate(457).name + ", \n"
        + "|5|Top 6: "
        + "x" + 5 + " " + ItemService.gI().getTemplate(457).name + ", \n"
        + "|7|Top 7: "
        + "x" + 4 + " " + ItemService.gI().getTemplate(457).name + ", \n"
        + "|7|Top 8: "
        + "x" + 3 + " " + ItemService.gI().getTemplate(457).name + ", \n"
        + "|7|Top 9: "
        + "x" + 2 + " " + ItemService.gI().getTemplate(457).name + ", \n"
        + "|7|Top 10: "
        + "x" + 1 + " " + ItemService.gI().getTemplate(457).name + ", \n"
        + "Nhận quà sau mỗi 00h15 hàng ngày!"

    };

    private static void traoQuaTopI(int i, int id) {
        try {

            Player pl = null;
            if (pl == null) {
                pl = GodGK.loadPlayerByID(id);
            }
            ...
            switch (i) {
                case 0: {
                    ...
                    Logger.error("Đã trao quà top " + (i + 1) + " TV cho: " + pl.name);
                    break;
                }
                case 1: {
                    Item item = ItemService.gI().createNewItem((short) 457);
...
//            Logger.success("Da trao qua " + i);
            ...
            e.printStackTrace();
            Logger.success("Loi trao top NHS " + i + " cho id: " + id);
        }
    }
