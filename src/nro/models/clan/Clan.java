...
public class Clan {
    ...
    public boolean gas_haveGone;
    public Player playerOpenKhiGas;
    public Gas khiGas;

    public long ConDuongRanDoc_lastTimeOpen;
    public boolean ConDuongRanDoc_haveGone;
    public ConDuongRanDoc ConDuongRanDoc;
    public String CDRD_playerOpen;
    ...
    public void insert() {
        ...
        for (ClanMember cm : this.members) {
            ...
            dataObject.put("power_point", cm.powerPoint);
...
//            dataArray.add(dataObject.toJSONString());
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ps = con.prepareStatement("insert into clan_sv" + Manager.SERVER
                    + "(id, name, slogan, img_id, power_point, max_member, clan_point, level, members, doanh_trai, ban_do_kho_bau, ran_doc) "
                    + "values (?,?,?,?,?,?,?,?,?,?,?,?)");
            ps.setInt(1, this.id);
            ...
            ps.setString(10, "[" + System.currentTimeMillis() + ",\"Doanhtrai\"]");
            ps.setString(11, "[" + System.currentTimeMillis() + ",\"bdkb\"]");
            ps.setString(12, "[" + System.currentTimeMillis() + ",\"randoc\"]");
            ps.executeUpdate();
            ps.close();
        } catch (Exception e) {
