...
public class DaiHoiVoThuatService {
    ...
    public void Update() {
        ...
        int countNull = 0;
        int maxZone = 0;
        if(Util.contains(daihoi.Time, String.valueOf(DaiHoiVoThuat.getInstance().Hour)) && daihoi.round != 1 ){
        ...
        daihoi.listReg.stream().filter((me) -> (me != null && me.zone.map.mapId != 52 && me.zone.map.mapId != 51 && DaiHoiVoThuat.getInstance().Minutes >= daihoi.min_start_temp)).map((me) -> {
            Service.gI().sendThongBao(me, "Bạn đã bị tước quyền thi đấu do không có mặt kịp giờ");
            return me;
        }).forEachOrdered((me) -> {
            daihoi.listReg.remove(me);
        });
        if (DaiHoiVoThuat.getInstance().Second == 0 && DaiHoiVoThuat.getInstance().Minutes < daihoi.min_start_temp) {
            if (daihoi.listReg.size() > 1) {
                Service.gI().sendThongBao(daihoi.listReg, "Vòng " + daihoi.round + " sẽ bắt đầu sau " + (daihoi.min_start_temp - DaiHoiVoThuat.getInstance().Minutes) + " phút nữa");
            }
        } else if (DaiHoiVoThuat.getInstance().Minutes >= daihoi.min_start) {
            if (daihoi.listReg.size() > 1) {
                if (daihoi.listReg.size() % 2 == 0) {
                    if(DaiHoiVoThuat.getInstance().Minutes >= daihoi.min_start_temp){
                        MatchDHVT();
                    }
                } else {
    ...
    public boolean CanReg(Player pl) {
        return daihoi != null &&pl.isPl()&& Util.contains(daihoi.Time, String.valueOf(DaiHoiVoThuat.getInstance().Hour)) && DaiHoiVoThuat.getInstance().Minutes <= daihoi.min_limit && !PlayerExits((int)pl.id);
    }
    
    public String Giai(Player pl) {
        if (daihoi != null && PlayerExits((int)pl.id)) {
            return "Đại hội võ thuật sẽ bắt đầu sau "+(daihoi.min_start_temp - DaiHoiVoThuat.getInstance().Minutes)+" phút nữa";
        } else if (daihoi != null && Util.contains(daihoi.Time, String.valueOf(DaiHoiVoThuat.getInstance().Hour)) && DaiHoiVoThuat.getInstance().Minutes <= daihoi.min_limit) {
