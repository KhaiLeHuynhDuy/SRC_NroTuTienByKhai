...
//                System.err.println("sss " + item.getConsignID());
    ...
    public void close() {
        try (Connection con = GirlkunDB.getConnection(); 
                PreparedStatement truncateStatement = con.prepareStatement("TRUNCATE consignment_shop"); PreparedStatement insertStatement = con.prepareStatement("INSERT INTO `consignment_shop`(`id`, `consignor_id`, `tab`, `item_id`, `gold`, `gem`, `quantity`, `item_options`, `up_top`, `sold`,`time_consign`) VALUES (? ,?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
            ...
            for (ConsignmentItem it : list) {
                if (it != null) {
                    ...
                    insertStatement.setInt(1, it.getConsignID());
